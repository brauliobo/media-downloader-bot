...
module Audiobook::Ewprs
  class Catalog
    ...
    VOICE_ACCENTS  = {'en' => 'neutral English accent', 'pt' => 'portuguese accent'}.freeze
    ...
    def initialize(root, language: 'en')
    ...
    def discourses
      @discourses ||= begin
        ...
        nav.css('.list_discourse_title').filter_map do |node|
          ...
          translated_sources = details.css('.discourse_box_references a[href]').map do |item|
            localized_title(local_ref(item['href'], details.url).first, normalize(item.text))
          end
          Entry.new(
            kind:      :discourse,
            title:     localized_title(path, entry_text(details, '.discourse_title', link.text)),
            path:      path,
            info:      entry_text(details, '.discourse_info', info_node&.text),
            sources:   translated_sources.empty? ? Array(refs_node&.css('a')).map { |item| normalize(item.text) } : translated_sources,
            book_refs: refs
          )
        end.uniq(&:path)
      end
    ...
    def books
      @books ||= begin
        ...
        nav.css('.books_book_title_tocs').filter_map do |node|
          link = node.at('a[href]')
          next unless link
          next if normalize(link.text).match?(UNAVAILABLE_BOOK)
          ...
          Entry.new(
            kind: :book, title: localized_title(path, entry_text(details, '.book_title', link.text)),
            path: path, chapters: chapters
          )
        end.uniq(&:path)
      end
    end
    ...
    def parse_options(entry)
      ...
      options = SymMash.new(
        ...
        instruct:              "male, middle-aged, moderate pitch, #{VOICE_ACCENTS.fetch(language, "#{language} accent")}"
      )
    ...
    def entry_text(details, selector, fallback)
      text = details.at_css(selector)&.text.to_s
      normalize(text.empty? ? fallback : text)
    end

    def localized_title(path, title)
      slug = File.basename(path, File.extname(path))
      I18n.t("audiobook.ewprs.titles.#{slug}", locale: language, default: title)
    end
