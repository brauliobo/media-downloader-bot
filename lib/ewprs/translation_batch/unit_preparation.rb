  ...
  class TranslationBatch
    module UnitPreparation
      ...
      def canonicalize_placeholders(source, tokens)
        ...
        aliases = resolved.to_h { |marker, value| [marker, first_marker_by_value[value] ||= marker] }
      ...
      def mask_coordinated_parenthetical_glosses(source)
        source.gsub(COORDINATED_PARENTHETICAL_GLOSSES) do |match|
          ...
          first = nested_parenthetical_gloss(first_term, captures[:first_spacing], captures[:first_gloss])
          second = nested_parenthetical_gloss(second_term, captures[:second_spacing], captures[:second_gloss])
          "#{first}#{captures[:coordination]}#{second}"
        end
      end
