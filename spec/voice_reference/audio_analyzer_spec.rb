...
RSpec.describe VoiceReference::AudioAnalyzer do
  it 'keeps the raw reference audio by default' do
    ...
    expect(command).not_to include('-af')
  end

  it 'keeps the full voice-quality filter available explicitly' do
    status = instance_double(Process::Status)
    command = nil
    allow(Sh).to receive(:run) do |value|
      command = value
  ...
  it 'supports padded extraction at a caller-selected sample rate' do
    ...
    filter = command.fetch(command.index('-af') + 1)
    expect(filter).to include(described_class::CLONE_REFERENCE_FILTER, 'apad=pad_dur=0.15')
