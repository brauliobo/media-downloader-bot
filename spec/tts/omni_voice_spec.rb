...
RSpec.describe TTS::OmniVoice do
  it 'sends temperature zero as an OmniVoice API parameter by default' do
    Dir.mktmpdir('omnivoice-spec-') do |dir|
      ...
      expect(captured_form['temperature']).to eq('0')
  ...
  it 'allows callers to override OmniVoice temperature' do
    Dir.mktmpdir('omnivoice-spec-') do |dir|
      ...
      expect(captured_form['temperature']).to eq('0.3')
    end
  end
  ...
  it 'accepts temp as the user-facing alias for temperature' do
    Dir.mktmpdir('omnivoice-spec-') do |dir|
      ...
      expect(captured_form['temperature']).to eq('0.2')
